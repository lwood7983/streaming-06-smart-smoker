...
def food_B_callback(ch, method, properties, body):
    """" Define behavior on getting a message about the foodB temperature.  Since the producer 
    ...
    food_b_temp_deque.append(message)

    # identifying first item in the deque
    food_b_deque_temp = food_b_temp_deque[0]
...
def main(hn: str, qn: str):
    ...
    try:
        ...
        channel.queue_declare(queue=food_b_temp_queue, durable=True)
        ...
        channel.basic_consume( queue=food_b_temp_queue, on_message_callback=food_B_callback)
